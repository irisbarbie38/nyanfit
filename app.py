...
def create_app():
    ...
    @login_required
    def api_stats():
        # Monday 00:00 in the server timezone is sufficient for this personal tracker.
        now = datetime.now().astimezone()
        week_start = now.replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(days=now.weekday())
        sets_query = SetLog.query.filter(SetLog.user_id == session["user_id"], SetLog.created_at >= week_start)
        sets = sets_query.count()
        volume = db.session.query(func.coalesce(func.sum(SetLog.weight * SetLog.reps), 0)).filter(
            SetLog.user_id == session["user_id"], SetLog.created_at >= week_start
        ).scalar()
        ...
        started = WorkoutSession.query.filter(
            WorkoutSession.user_id == session["user_id"],
            WorkoutSession.started_at >= week_start,
        ).count()
        completion = (sessions / started * 100) if started else 0
        return jsonify(sets=sets, volume=float(volume or 0), sessions=int(sessions), completion=completion)
